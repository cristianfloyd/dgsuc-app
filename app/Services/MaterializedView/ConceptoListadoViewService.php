...
class ConceptoListadoViewService
{
    ...
    public function exists(): bool
    {
        try {
            ...
            Log::error('Error verificando vista materializada', [
                'error' => $e->getMessage()
            ]);
            return false;
        }
        ...
        try {
            ...
            DB::connection($this->getConnectionName())->transaction(function (){
                ...
                DB::connection($this->getConnectionName())->statement(
                    query: "CREATE MATERIALIZED VIEW IF NOT EXISTS suc.concepto_listado AS
                ...
                DB::connection($this->getConnectionName())->statement('CREATE INDEX IF NOT EXISTS idx_concepto_listado_codn_conce ON suc.concepto_listado(codn_conce)');
                DB::connection($this->getConnectionName())->statement('CREATE INDEX IF NOT EXISTS idx_concepto_listado_periodo_fiscal ON suc.concepto_listado(periodo_fiscal)');
            });
    ...
    public function refresh(): void
    {
        try {
            ...
            Log::info('Vista materializada concepto_listado actualizada exitosamente');
        } catch (\Throwable $th) {
