...
class Gerencial
{
static function generar_datos_gerencial_nuevo($tabla_liqui, $where='TRUE')
	{
		//-- Elimino las temporales que utilizo por si existen
		$sql = "DROP TABLE IF EXISTS datos_base_dh21;
				DROP TABLE IF EXISTS importes_netos;
				DROP TABLE IF EXISTS datos_antiguedad;
				DROP TABLE IF EXISTS datos_trabajados;";

		//-- Obtengo los datos b�sicos de dh21 sobre los cuales voy a
		...
		$sql .=	"SELECT DISTINCT
					...
					dh03.codc_agrup,
					dh03.hs_dedic
					INTO TEMP TABLE datos_base_dh21
				FROM
		...
		$sql .="SELECT
					...
					INTO TEMP TABLE importes_netos
				FROM
					datos_base_dh21 db
		...
		$sql .="SELECT
					...
					INTO TEMP TABLE datos_antiguedad
				FROM
					" . MAP_ESQUEMA . ".$tabla_liqui dh21
					JOIN " . MAP_ESQUEMA . ".dh22 ON (dh22.nro_liqui = dh21.nro_liqui)
					LEFT OUTER JOIN " . MAP_ESQUEMA . ".dh01 ON (dh01.nro_legaj = dh21.nro_legaj)
		...
		//-- Obtengo las horas catedra y los d�as trabajados
