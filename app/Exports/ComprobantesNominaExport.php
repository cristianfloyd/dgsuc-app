...
class ComprobantesNominaExport implements FromCollection, WithHeadings, WithTitle, WithStyles, WithEvents, WithCustomStartCell, ShouldAutoSize, WithDrawings
{
    protected $nroLiqui;
    protected $descLiqui;
    ...
    protected function getImporteEnLetras(): string 
    {
        ...
        return mb_strtoupper($formatter->format($this->totalImporte));
    }
    ...
    public function registerEvents(): array
    {
        return [
            BeforeSheet::class => function (BeforeSheet $event) {
                ...
                $event->sheet->mergeCells('A1:B3');
                $event->sheet->mergeCells('A4:B6');

                // Título
                $event->sheet->mergeCells('A7:B7');
                $event->sheet->setCellValue('A7', 'Orden de Pago de Descuento DEFINITIVA ' . $this->nroLiqui);
    ...
    public function collection()
    {
        return ComprobanteNominaModel::where('nro_liqui', $this->nroLiqui)
            ->select('descripcion_retencion as Descripción', 'importe as Importe')
            ->get();
    }
