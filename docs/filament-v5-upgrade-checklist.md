...
**Estado:** En curso. Marca cada ítem al completarlo.
...
|-----------|------------|------------|
| Tailwind CSS v4.0+ | ✅ tailwindcss 4.2.1 | [x] |
| Livewire v4.0+ | ✅ livewire 4.2.1 | [x] |
| Laravel v11.28+ | ✅ Laravel 12.x | [x] |
| PHP 8.2+ | ✅ PHP ^8.4 | [x] |
...
- [x] Ejecutar: `docker compose exec app composer require livewire/livewire:^4.0`
- [x] Ejecutar: `docker compose exec app php artisan optimize:clear`

### 2.2 Actualizar `config/livewire.php`

Cambios principales (v3 → v4):

- [x] **Layout:** `'layout' => 'components.layouts.app'` → `'component_layout' => 'layouts::app'`  
  (o el layout que uses; el valor por defecto v4 es `layouts::app` → `resources/views/layouts/app.blade.php`).
- [x] **Placeholder:** `'lazy_placeholder'` → `'component_placeholder'` (mismo valor, ej. `'livewire.placeholder'` o `null`).
- [x] Revisar si quieres añadir: `'smart_wire_keys' => true` (por defecto en v4).
- [x] Revisar nuevas opciones opcionales: `component_locations`, `component_namespaces`, `make_command`, `csp_safe` (ver guía).

### 2.3 Rutas de componentes full-page (si aplica)

- [x] Si usas `Route::get('/ruta', Componente::class)`, valorar cambiar a `Route::livewire('/ruta', Componente::class)` (recomendado en v4).

### 2.4 Comportamiento de `wire:model`

- [x] Si usas `wire:model` en contenedores que reciben eventos de hijos, considerar añadir el modificador `.deep` donde haga falta.
- [x] Si usas `wire:model.blur` o `wire:model.change` y quieres el comportamiento anterior a v4, usar `wire:model.live.blur` / `wire:model.live.change`.

### 2.5 Componentes Livewire en Blade

- [x] Asegurar que los tags de componentes Livewire estén **cerrados** (ej. `<livewire:nombre />` en lugar de `<livewire:nombre>` sin cerrar).

### 2.6 Scroll con `wire:navigate`

- [x] Si usas `wire:scroll` para preservar scroll con `wire:navigate`, cambiar a `wire:navigate:scroll`.

### 2.7 Verificación Livewire v4

- [x] `docker compose exec app composer show livewire/livewire` muestra versión ^4.x.
...
- [x] Si usas otros plugins de Filament, comprobar en Packagist/GitHub si tienen versión compatible con Filament v5.
...
- [x] Ejecutar los comandos que imprima el script (por ejemplo):
  ```bash
...
- [x] Ejecutar: `docker compose exec app composer require filament/spatie-laravel-media-library-plugin:"^5.0" -W`
