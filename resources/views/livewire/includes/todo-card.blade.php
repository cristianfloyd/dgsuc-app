<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-gray-500 col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex items-center">
            ...
            @else
                <input wire:click="toggle({{$todo->id}})" type="checkbox" class="form-checkbox h-5 w-5 text-teal-600">
            ...
            <input wire:model="editingName" type="text" placeholder="Todo..."
                ...
                @error('editingName')
                    <div><span class="mt-1 ml-1 text-red-500 text-xs block">{{ $message }}</span></div>
                @enderror

            @else
                <h3 class="ml-3 text-lg text-semibold text-gray-800">{{ $todo->name }}</h3>
            @endif
        </div>
    ...
    <div class="mt-3 text-xs text-gray-700">
        @if ($editingTodoId == $todo->id)
                <button wire:click="update"
                    class="mt-3 px-4 py-2 bg-teal-500 text-white font-semibold rounded hover:bg-blue-600">Update</button>
